...
export class MangaListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.displayMangaList = this.mangaList?.slice(
      ...
      (this.currentPage + 1) * this.paginationLimit,
    )
    console.log('??')
  }
  ...
  nextPage() {
    this.currentPage += 1
    if (this.currentPage * this.paginationLimit >= this.mangaList!.length)
      this.currentPage = 0
    this.ngOnChanges()
  }
  ...
  previousPage() {
    ...
    if (this.currentPage < 0)
      this.currentPage = this.mangaList!.length - this.paginationLimit - 1
    this.ngOnChanges()
  }
}
